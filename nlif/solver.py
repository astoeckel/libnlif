...
class Solver:
    ...
    def _compile_library(debug=False, parallel_compile=True):
        ...
        return cmodule.SharedLibrary(soname)

    def __init__(self, debug=False, parallel_compile=True):
        # Assemble a key that uniquely identifies the library that we would
        # like to compile
        _dll_key = json.dumps({"debug": debug}, sort_keys=True)

        # Compile the library if this had not happened yet
        if not _dll_key in Solver._dlls:
            Solver._dlls[_dll_key] = Solver._compile_library(
                debug=debug, parallel_compile=parallel_compile)
        self._dll = Solver._dlls[_dll_key]
    ...
    def nlif_solve_parameters_iter(self,
        ...
        with SigIntHandler() as sig_int_handler:
            ...
            if err != 0:
                raise RuntimeError(self._dll.bioneuronqp_strerr(err))
